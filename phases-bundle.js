      ...
      ${
        ...
        <div class="stitch" style="padding:16px;${p.blocked ? 'opacity:.7;border-color:var(--danger)' : ''}">
          ...
          <div class="row-actions" style="flex-wrap:wrap">
            ...
            ${Math.abs(bal) < 0.01
              ? `<button class="btn btn-danger btn-sm" onclick="deletePartyIfClear('${p.id}')">Delete</button>`
              : `<button class="btn btn-outline btn-sm" onclick="clearLedgerBalance('party','${p.id}')">Set balance 0</button>`}
          </div>
        </div>`;
              })
              .join('')
